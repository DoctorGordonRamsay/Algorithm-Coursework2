...
public class ContactsHashOpen implements IContactDB {  
    private final int initialTableCapacity = 2003;
    ...
    private int findPos(String name) {
        ...
        while (table[pos] != null && !name.equals(table[pos].getName())) {
           ...
           numVisited++;
           pos = (pos + i * i) % table.length; // linear probing
           i++;
        }  
    ...
    public Contact remove(String name) {
        ...
        if (table[pos] != null && table[pos].getName().equals(name)) {
            RemovedItem = table[pos];
            table[pos] = new Contact("____","____");
            numEntries--;
            return RemovedItem;
    ...
    public void displayDB() {
        ...
        int j = 0;
        List<Contact>[] toBeSortedTable = (LinkedList<Contact>[]) new LinkedList[tableCapacity];  // OK to use Array.sort
        
        for (int i = 0; i != table.length; i++) {
            int hash = (7 * 23 + i)  % table.length;
            if (table[i] != null) {
                if (toBeSortedTable[hash] == null) {
                    toBeSortedTable[hash] = new LinkedList<>();
                }
                toBeSortedTable[hash].add(table[i]);
                j++;
            }
        }
