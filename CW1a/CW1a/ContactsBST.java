...
public  class ContactsBST implements IContactsDB {
    ...
    public void handleMessage(String input) {
        switch (input.toLowerCase()) {
            ...
            case "search":
                ...
                break;
            default:
                System.out.println("Gay porn pls"); // Your custom message
                break;
        }
    }
   ...
   public ContactsBST() {
    ...
    private Contact retrieve(Node tree, String name) {
        ...
        int value = name.compareTo(tree.data.getName());
        if (value == 0) {
            return tree.data;
        } else if (value > 0) {
    ...
    private Node insert(Node tree, Contact employee) {
        ...
        int cmp = employee.getName().compareTo(tree.data.getName());
        if (cmp > 0) {
            tree.right = insert(tree.right, employee);
        } else if (cmp < 0) {
            tree.left = insert(tree.left, employee);
        } else {
            tree.data = employee; // overwrite existing
        }
        return tree;
    }
    ...
    */
    ...
    private void traverse(Node tree) {
        if (tree != null) {
            traverse(tree.left);
            System.out.println(tree.data.getName() + " - " + tree.data.getAffiliation());
            traverse(tree.right);
        }
    }
